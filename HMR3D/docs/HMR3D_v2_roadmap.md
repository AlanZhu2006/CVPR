...
下面这些我已经在当前 `cuVSLAM -> HMR3D` 本地分支里落下来了，且都做过同条件 replay 对比：

- 已落地：`write-aware archive policy`
- 已落地：`anchor retention`
- 已落地：`pose-anchor gate`
- 已落地：`shadow recover`
- 已落地：`scene-level hierarchical bank`
- 已落地：`query/readout routing`
- 已落地：`multi-candidate merge`
- 已落地：`local adaptation`
- 已落地：`incremental active Gaussian submap builder`
- 已补齐：统一配置入口、对比脚本、SVG 可视化

当前推荐对比结果：

- baseline: [kitti06_baseline_v1_cfgfix/summary.json](/home/nyu/Codespace/CVPR/HMR3D/nuc_output/kitti06_baseline_v1_cfgfix/summary.json:1)
- v2 tuned: [kitti06_v2_tuned_cfgfix/summary.json](/home/nyu/Codespace/CVPR/HMR3D/nuc_output/kitti06_v2_tuned_cfgfix/summary.json:1)
- v3 full: [kitti06_v3_full_cfgfix/summary.json](/home/nyu/Codespace/CVPR/HMR3D/nuc_output/kitti06_v3_full_cfgfix/summary.json:1)

这三版的最重要趋势是：

- `baseline -> v2 tuned`：archive 从 `76` 压到 `24`
- `v2 tuned -> v3 full`：recover 从 `7` 提到 `8`
- `v2 tuned -> v3 full`：pose-anchor reject 从 `37` 降到 `29`
- `v2 tuned -> v3 full`：scene summaries 从 `24` 压到 `7`
- `v2 tuned -> v3 full`：出现 `merge_events = 4`、`local_adapt_applied = 8`
- `v3 full -> v4 gaussian smoke`：新增 `gaussian_archives = 7`、`gaussian_archived_points_total = 420`
- `v4 gaussian long replay`：`gaussian_warmstart_requests = 8`、`gaussian_warmstart_points = 766`
