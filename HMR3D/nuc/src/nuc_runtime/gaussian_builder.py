...
class IncrementalGaussianBuilder:
    ...
    def _lingbot_dense_geometry_to_gaussian_bundle(
        ...
        point_uv = dense["point_uv"].astype(np.float32) if "point_uv" in dense else None
        point_depth = dense["point_depth"].astype(np.float32) if "point_depth" in dense else None
        ...
        image_height = 0
        image_width = 0
        if "depth" in dense and dense["depth"].ndim >= 3:
            image_height = int(dense["depth"].shape[1])
            image_width = int(dense["depth"].shape[2])
        ...
        if point_uv is not None and point_uv.shape != (count, 2):
            point_uv = None
        if point_depth is not None and point_depth.shape[0] != count:
            point_depth = None
        ...
        if valid_idx.size > self.config.gaussian_structured_max_points:
            ...
            keep_local = self._select_balanced_indices(
                ...
                point_uv=point_uv[valid_idx].astype(np.float32) if point_uv is not None else None,
                point_depth=point_depth[valid_idx].astype(np.float32) if point_depth is not None else None,
    ...
    def _select_balanced_indices(
        ...
        for frame_id in unique_frames:
            ...
            top_local = self._select_frame_diverse_indices(
                score=score[frame_idx].astype(np.float32),
                point_uv=point_uv[frame_idx].astype(np.float32) if point_uv is not None else None,
                point_depth=point_depth[frame_idx].astype(np.float32) if point_depth is not None else None,
                image_width=image_width,
                image_height=image_height,
                max_points=take_count,
            )
            top = frame_idx[top_local]
    ...
    def _select_frame_diverse_indices(
        self,
        score: np.ndarray,
        point_uv: np.ndarray | None,
        point_depth: np.ndarray | None,
        ...
        if max_points <= 0 or score.size == 0:
            return np.zeros((0,), dtype=np.int32)
        if score.size <= max_points:
            return np.arange(score.size, dtype=np.int32)
        if point_uv is None or point_uv.shape != (score.size, 2):
            keep = np.argpartition(score, -max_points)[-max_points:]
            return keep[np.argsort(score[keep])[::-1]].astype(np.int32)

        width = float(image_width) if image_width > 1 else float(np.nanmax(point_uv[:, 0]) + 1.0)
        height = float(image_height) if image_height > 1 else float(np.nanmax(point_uv[:, 1]) + 1.0)
        width = max(width, 1.0)
        height = max(height, 1.0)

        col_bins = np.clip((point_uv[:, 0] / width * 4.0).astype(np.int32), 0, 3)
        row_bins = np.clip((point_uv[:, 1] / height * 3.0).astype(np.int32), 0, 2)
        depth_bins = np.zeros((score.size,), dtype=np.int32)
        if point_depth is not None and point_depth.shape[0] == score.size:
            finite_depth = np.isfinite(point_depth) & (point_depth > 0.0)
            if int(np.count_nonzero(finite_depth)) >= 8:
                near_cut, far_cut = np.quantile(point_depth[finite_depth], [0.35, 0.7])
                depth_bins[finite_depth & (point_depth > near_cut)] = 1
                depth_bins[finite_depth & (point_depth > far_cut)] = 2

        bucket_ids = row_bins + (3 * col_bins) + (12 * depth_bins)
        return self._select_bucket_round_robin(
            score=score.astype(np.float32),
            bucket_ids=bucket_ids.astype(np.int32),
            max_points=max_points,
        )

    def _select_bucket_round_robin(
        self,
        score: np.ndarray,
        ...
        bucket_orders: list[np.ndarray] = []
        for bucket_id in np.unique(bucket_ids):
            member_idx = np.flatnonzero(bucket_ids == int(bucket_id))
            if member_idx.size == 0:
                continue
            member_score = score[member_idx]
            ordered = member_idx[np.argsort(member_score)[::-1]]
            bucket_orders.append(ordered.astype(np.int32))

        if not bucket_orders:
            keep = np.argpartition(score, -max_points)[-max_points:]
            return keep[np.argsort(score[keep])[::-1]].astype(np.int32)

        positions = np.zeros((len(bucket_orders),), dtype=np.int32)
        active = list(range(len(bucket_orders)))
        selected: list[int] = []
        while active and len(selected) < max_points:
            active.sort(
                key=lambda bucket_idx: float(score[bucket_orders[bucket_idx][positions[bucket_idx]]]),
                reverse=True,
            )
            next_active: list[int] = []
            for bucket_idx in active:
                if len(selected) >= max_points:
                    break
                member_order = bucket_orders[bucket_idx]
                member_pos = int(positions[bucket_idx])
                if member_pos >= member_order.size:
                    continue
                selected.append(int(member_order[member_pos]))
                member_pos += 1
                positions[bucket_idx] = member_pos
                if member_pos < member_order.size:
                    next_active.append(bucket_idx)
            active = next_active
