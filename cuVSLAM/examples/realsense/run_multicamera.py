...
def detect_device_model() -> str:
    """Read the Jetson model string when available."""
    for model_path in (
...
def resolve_config_file(config_path: Optional[str]) -> Path:
    """Resolve the rig configuration path, defaulting by detected Jetson model."""
    ...
    model = detect_device_model().lower()
    if "orin nano" in model:
        return base_dir / "frame_nano_rig.yaml"
    if "thor" in model:
        return base_dir / "frame_thor_rig.yaml"
    return base_dir / "frame_agx_rig.yaml"


def load_camera_configuration(config_path: Path) -> Dict:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run cuVSLAM multi-camera RealSense tracking."
    )
    parser.add_argument(
        "--rig-config",
        default=os.environ.get("CUVSLAM_RS_RIG_FILE"),
        help=(
            "Rig YAML file to use. Defaults to frame_nano_rig.yaml on Orin Nano, "
            "frame_thor_rig.yaml on Thor, otherwise frame_agx_rig.yaml."
        ),
    )
    parser.add_argument(
        "--warmup-frames",
        ...
        help="Number of frames to skip while waiting for inter-camera sync.",
    )
...
def main() -> None:
    ...
    args = parse_args()
    config_path = resolve_config_file(args.rig_config)
    if not config_path.exists():
        raise FileNotFoundError(f"Rig configuration not found: {config_path}")

    device_model = detect_device_model()
    if device_model:
        print(f"Detected device model: {device_model}")

    # Load configuration
    config_data = load_camera_configuration(config_path)
