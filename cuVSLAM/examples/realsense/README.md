...
On Jetson Orin Nano, `run_multicamera.py` now auto-selects [frame_nano_rig.yaml](frame_nano_rig.yaml) when the device model contains `Orin Nano`. You can also override the rig file explicitly:

```bash
python3 run_multicamera.py --rig-config frame_nano_rig.yaml
```

On a headless Jetson, save the visualization to a Rerun recording instead of trying to spawn a desktop viewer:

```bash
CUVSLAM_RERUN_MODE=save python3 run_multicamera.py --rig-config frame_nano_rig.yaml
```

This writes a `.rrd` file under `examples/realsense/recordings/` by default. To serve the live viewer over HTTP instead, use `CUVSLAM_RERUN_MODE=web`.
