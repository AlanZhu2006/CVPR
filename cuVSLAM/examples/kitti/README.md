...
On headless Jetson systems, save the visualization as a Rerun recording instead of spawning a desktop viewer:

```bash
CUVSLAM_RERUN_MODE=save python3 track_kitti_slam.py
```

This writes `outputs/kitti/track_kitti_slam.rrd` by default. To view the recording in a browser instead, run with `CUVSLAM_RERUN_MODE=web`.
For offline Jetson usage, the script defaults to synchronous SLAM for stability. Use `--async-slam` if you want the original asynchronous behavior.

To fetch only sequence `06` instead of downloading the full 22 GB archive locally, use:

```bash
python3 download_kitti_sequence.py --sequence 06
```

This script extracts `dataset/sequences/06` from the public KITTI grayscale mirror archive using HTTP range requests.
